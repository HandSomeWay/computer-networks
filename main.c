...
void log_message(const char *message, ...)
{
	...
	char vbuffer[255];
	va_list args;// 指向参数的指针的变量
	va_start(args, message);// 用va_start宏初始化变量
	vsnprintf(vbuffer, ARRAY_SIZE(vbuffer), message, args);// 执行成功，返回生成字符串长度，若生成长度大于size，则前size个字符复制到str，将原串的长度返回; 执行失败，返回负值，并置errno
	va_end(args); //可变参数的获取
...
int app_connect(int type, void *buf, unsigned short int portnum)
{
	...
	if (type == IP) {
		...
		if (connect(fd, (struct sockaddr *)&remote, sizeof(remote)) < 0) { // 建立连接
			...
			//要给sa_data赋值就必须同时指明IP地址和端口号，但没有相关函数将这个字符转换成所需要的形式,所以使用sockaddr_in来代替，两个结构体长度相同，强制转换类型时不会丢失字节
...
void app_socket_pipe(int fd0, int fd1)
{
	...
	while (1) {
		...
		FD_SET(fd0, &rd_set);// 用于在文件描述符集合中增加一个新的文件描述符
		FD_SET(fd1, &rd_set);
